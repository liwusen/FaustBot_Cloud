...
ROOT key 用于管理 Service Key，本身不参与普通 TTS/ASR 调用。先在 CLI 中设置：

```powershell
D:/dev/faustbot/faust/.runtime/python.exe cloud_inference_server/cli.py set-root-key FSK-ROOT-xxxxxxxxxxxxxxxxxxxx
```

随后管理接口使用 `Authorization: Bearer <ROOT_KEY>` 或 `X-ROOT-Key` 头即可。

### 管理 API

```powershell
curl -X POST "http://127.0.0.1:18980/v1/admin/keys" ^
  -H "Authorization: Bearer FSK-ROOT-xxxxxxxxxxxxxxxxxxxx" ^
  -F "name=demo" ^
  -F "note=local-test"
```

```powershell
curl -X POST "http://127.0.0.1:18980/v1/admin/keys/FSK-xxxxxxxxxxxxxxxxxxxxxxx/disable" ^
  -H "Authorization: Bearer FSK-ROOT-xxxxxxxxxxxxxxxxxxxx"
```

```powershell
curl -X POST "http://127.0.0.1:18980/v1/admin/keys/FSK-xxxxxxxxxxxxxxxxxxxxxxx/enable" ^
  -H "Authorization: Bearer FSK-ROOT-xxxxxxxxxxxxxxxxxxxx"
```

```powershell
curl -X POST "http://127.0.0.1:18980/v1/admin/keys/FSK-xxxxxxxxxxxxxxxxxxxxxxx/reset-usage" ^
  -H "Authorization: Bearer FSK-ROOT-xxxxxxxxxxxxxxxxxxxx"
```
